...
from utils.function import EMAModel
from models.resnet import encoder_linear
from utils.function import ProtoCrossAttn
from utils.function import dual_mlp
from utils.function import ConfidenceLabelLoss
from utils.function import PrototypeSupConLoss
from nfp_bank_dual import cad_loss
from dataset import CIFAR10
parser = argparse.ArgumentParser(description='Adversarial Training')
parser.add_argument('--train-batch-size', type=int, default=128, metavar='N', help='input batch size for training (default: 128)')
parser.add_argument('--test-batch-size', type=int, default=100, metavar='N', help='input batch size for testing (default: 128)')
...
parser.add_argument('--theta1', type=int, default=6, metavar='S', help='adapter loss')
parser.add_argument('--theta2', type=int, default=6, metavar='S', help='adapter loss')
parser.add_argument('--proto-num', type=float, default=10, metavar='M', help='prototype tensor')
parser.add_argument('--proto-dim', type=float, default=512, metavar='M', help='prototype tensor')
parser.add_argument('--proto-momentum', type=float, default=0.9, metavar='M', help='prototype tensor')
...
parser.add_argument('--model-dir', default='./nfp_bank_dual', help='directory of model for saving checkpoint')
...
def main(args):
    ...
    logger.log('\n')
    for epoch in range(41, args.epochs + 1):
        ...
        test_natural_accuracy, test_pgd_accuracy = eval_pgd_whitebox(encoder, classifier, test_loader, num_steps=args.test_pgd_steps, step_size=args.pgd_step_size)
        test_natural_accuracy, test_fgsm_accuracy = eval_fgsm_whitebox(encoder, classifier, test_loader, num_steps=args.test_fgsm_steps, step_size=args.fgsm_step_size)
        test_end = time.time()
        logger.log(
